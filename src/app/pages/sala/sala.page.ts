...
export class SalaPage implements OnInit {
  ...
  async abandonarSala() {
    ...
    let indexJugador = this.partido.idsJugadores.findIndex(
      (idplayer) => idplayer == this.jugador.id
    );
    let dataSqlPartido = {
  ...
  async actualizarPartido(partidoActualizado) {
    ...
    console.log(
      '%cRETIRANDO JUGADOR DEL PARTIDO [' +
        partidoActualizado.idpartido +
        '] -----> ' +
        path,
      'color:black; background-color: yellow; font-size: 16px; font-weight: bold;'
    );
    return await (
      await fetch(requestSqlPartido, {
        method: 'PUT',
  ...
  async actualizarSala() {
    ...
    if (
      this.jugador.id &&
      !this.partido.idsJugadores.includes(this.jugador.id)
    ) {
    ...
    for (let i = 0; i < 10; i++) {
      if (this.partido.idsJugadores[i] !== 0) {
        this.jugadorAux = await this.descargarJugador(
          this.partido.idsJugadores[i]
        );
  ...
  repartirRedYBlue(updatePartido = false) {
    ...
    console.log(
      'ANALIZAR QUE this.jugadores SE ACTUALICE AL SALIR UN JUGADOR (que quede el lugar vacio)'
    );
    ...
    for (let i = 0; i < idsFiltradas.length; i++) {
      ...
      for (let j = 0; j < this.jugadores.length; j++) {
        ...
        i % 2 == 0
          ? (this.equipoRed[j / 2] = player)
          : (this.equipoBlue[Math.floor(j / 2)] = player);
      }
    }
    for (let i = 0; i < 5; i++)
      if (!this.equipoRed[i]) this.equipoRed[i] = { nombre: ' (disponible) ' };
    for (let i = 0; i < 5; i++)
      if (!this.equipoBlue[i])
  ...
  updateJugadoresSql() {
    ...
    let requestSqlPartido = 'https://backend-f1-java.herokuapp.com' + path;
    console.log(
      '%cACT JUGADORES DEL PARTIDO ID [' +
        dataSqlPartido.idpartido +
        '] -----> ' +
        path,
    ...
    fetch(requestSqlPartido, {
      ...
      .catch((err) =>
        this.presentToast(
          '💀 La última modificación al partido no logró completarse con éxito',
          3000
        )
      );
  }
  ...
  pagar() {
    ...
    let requestSqlJugador = 'https://backend-f1-java.herokuapp.com' + path;
    console.log(
      '%cACTUALIZAR JUGADOR SI PAGÓ [' +
        dataSqlJugador.nombre +
        '] -----> ' +
        path,
    ...
    fetch(requestSqlJugador, {
      ...
      .then((res) => res.json())
      .then(() =>
        this.presentToast('Se registró tu pago correctamente ✅', 1000)
      )
      .catch(() =>
        this.presentToast('💀 Hubo un error registrando tu pago', 5000)
      );
  }
  ...
  mezclarEquipos(arr1, arr2) {
    ...
    var currentIndex = arrAux.length,
      temporaryValue,
      randomIndex;
  ...
  fillStars(player, value) {
    player.stars = [];
    for (let i = 0; i < 5; i++) {
      if (value - 0.75 >= i) player.stars.push('full');
      else if (value - 0.25 >= i) player.stars.push('half');
