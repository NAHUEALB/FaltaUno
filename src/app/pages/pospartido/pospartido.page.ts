...
export class PospartidoPage implements OnInit {
  ...
  ionViewWillEnter() {
    this.storage.get('jugadores').then((jugadores) => {
      ...
      this.storage.get('partido').then((partido) => {
        ...
        this.salaEstado =
          this.partido.estado == ' Sala pública ' ? 'público 🔓' : 'privado 🔐';
        this.salaSexo =
          this.partido.sexo == ' No binario ' ? 'Mixto' : this.partido.sexo;
        this.repartirRedYBlue();
        this.iniciarStars();
      });
    });
  ...
  iniciarStars() {
    this.equipoRed.forEach(
      (jugador) => (jugador.stars = ['null', 'null', 'null', 'null', 'null'])
    );
    this.equipoBlue.forEach(
      (jugador) => (jugador.stars = ['null', 'null', 'null', 'null', 'null'])
    );
  }
  ...
  votarJugadorEspecifico(jugador, cantidad, team) {
    ...
    if (team === 'red') {
      ...
      this.votosRed[jugador] = {
        id: this.equipoRed[jugador].idjugador,
        valor: cantidad - 0 + 1,
      };
    } else {
      this.fillStars(this.equipoBlue[jugador], cantidad);
      this.votosBlue[jugador] = {
        id: this.equipoBlue[jugador].idjugador,
        valor: cantidad - 0 + 1,
      };
    }
  }
    ...
    for (let i = 0; i < this.votosRed.length; i++) {
      if (this.votosRed[i]) {
        this.descargarJugador(this.equipoRed[i].idjugador).then((jugador) => {
          jugador.puntaje =
            Number(jugador.puntaje) + Number(this.votosRed[i].valor);
    ...
    for (let i = 0; i < this.votosBlue.length; i++) {
      if (this.votosBlue[i]) {
        this.descargarJugador(this.equipoBlue[i].idjugador).then((jugador) => {
          jugador.puntaje =
            Number(jugador.puntaje) + Number(this.votosBlue[i].valor);
